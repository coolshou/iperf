...
struct iperf_settings {
  ...
  int cntl_ka_count;    /* Control TCP connection Keepalive number of retries
                           (TCP_KEEPCNT) */
  /* GSO/GRO fields always present to allow client-server negotiation regardless
   * of local support */
  int gso;
  int gso_dg_size;
  int gso_bf_size;
...
struct iperf_test {
  ...
  int max_server_duration; /* maximum possible duration of test as enforced by
                              the server (--max-server-duration flag) */
  ...
  void (*throughput_report_callback)(struct iperf_test *);
  // Add an opaque pointer to hold your Qt object instance
  void *user_data;
  Timer *omit_timer;
  Timer *timer;
  int done;
...
#define GSO_MAX_DG_IN_BF                                                       \
  (1 << 7UL) // 128 - the Linux Kernel limit hardcoded as `UDP_MAX_SEGMENTS (1
             // << 7UL)` in `udpgso.c`
