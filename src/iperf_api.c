...
void iperf_set_test_forceflush(struct iperf_test *ipt) { ipt->forceflush = 1; }
...
void iperf_set_test_json_stream_full_output(struct iperf_test *ipt,
...
void iperf_set_test_client_rsa_pubkey_from_file(
    struct iperf_test *ipt, const char *client_rsa_pubkey_file) {
  ipt->settings->client_rsa_pubkey =
      load_pubkey_from_file(client_rsa_pubkey_file);
}
...
void iperf_set_test_server_rsa_privkey_from_file(
    struct iperf_test *ipt, const char *server_rsa_privkey_file) {
  ...
  if (test->json_output) {
    cJSON_AddItemToObject(
        ...
        iperf_json_printf(
            ...
            "%d bidir: %d fqrate: %d interval: %f  gso: %d  gro: %d",
            ...
            test->stats_interval, (uint64_t)test->settings->gso,
            (uint64_t)test->settings->gro));
  } else {
    if (test->verbose) {
...
void iperf_on_connect(struct iperf_test *test) {
  ...
  if (test->json_output)
    cJSON_AddItemToObject(
        ...
        iperf_json_printf("time: %s  timesecs: %d  timemillisecs: %d", now_str,
                          (int64_t)now_secs, now_millisecs));
...
int iperf_parse_arguments(struct iperf_test *test, int argc, char **argv) {
  static struct option longopts[] = {
      ...
      {"json-stream-full-output", no_argument, NULL,
       OPT_JSON_STREAM_FULL_OUTPUT},
  ...
  while ((flag = getopt_long(
    ...
    case OPT_SERVER_MAX_DURATION:
      ...
      if (test->max_server_duration < 0 ||
          test->max_server_duration > MAX_TIME) {
        i_errno = IEDURATION;
        return -1;
      }
      server_flag = 1;
    ...
    case OPT_CNTL_KA:
      ...
      // Seems that at least in Windows WSL2, TCP keepalive retries full
      // interval must be smaller than the idle interval. Otherwise, the
      // keepalive message is sent only once.
    ...
    case OPT_GSRO:
      ...
      /* Flag is available regardless of local support to allow client to
       * request server to use it */
      gsro_flag = 1;
      test->settings->gso = 1;
      test->settings->gro = 1;
      break;
  ...
  if (gsro_flag) {
    ...
    warning("--gsro requested but UDP GSO not supported on this client; will "
            "be enabled on server if supported");
#elif !defined(HAVE_UDP_GRO)
    warning("--gsro requested but UDP GRO not supported on this client; will "
            "be enabled on server if supported");
#endif
  }
  ...
  if (test->protocol->id == Pudp && test->settings->gso) {
    ...
    if (test->settings->gso_dg_size > 0) {
      test->settings->gso_bf_size =
          (test->settings->gso_bf_size / test->settings->gso_dg_size) *
          test->settings->gso_dg_size;
    } else {
      /* If gso_dg_size is 0 (unlimited bandwidth), use default UDP datagram
       * size */
...
void iperf_check_throttle(struct iperf_stream *sp, struct iperf_time *nowP) {
  ...
  if (missing_rate < 0) {
    ...
    if (time_to_green_light >= 0) {
      ...
      // Sleep until average baud rate reaches the target value or interrupt /
      // error
      do {
...
static int send_parameters(struct iperf_test *test) {
  ...
  if (j == NULL) {
    ...
    /* Always send these fields to allow server to use GSO/GRO even if client
     * doesn't support it */
    if (test->protocol->id == Pudp) {
...
static int get_parameters(struct iperf_test *test) {
  ...
  if (j == NULL) {
    ...
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "sctp", cJSON_True)) != NULL)
      set_protocol(test, Psctp);
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "omit", cJSON_Number)) !=
        NULL) {
    ...
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "time", cJSON_Number)) !=
        NULL) {
    ...
    /* Always accept these fields to allow server to use GSO/GRO based on its
     * own support */
    ...
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "gso_dg_size", cJSON_Number)) !=
        NULL) {
      if (j_p->valueint < 0) {
        i_errno = IERECVPARAMS;
        r = -1;
    ...
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "gso_bf_size", cJSON_Number)) !=
        NULL) {
      if (j_p->valueint < 0) {
        i_errno = IERECVPARAMS;
        r = -1;
    ...
    /* Backward-compatibility: If client didn't send GSO params, derive from
     * blksize. */
    ...
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "gro_bf_size", cJSON_Number)) !=
        NULL) {
      if (j_p->valueint < 0) {
        i_errno = IERECVPARAMS;
        r = -1;
    ...
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "burst", cJSON_Number)) !=
        NULL) {
    ...
    if (test->protocol->id == Pudp &&
        (test->settings->blksize > 0 &&
         (test->settings->blksize < MIN_UDP_BLOCKSIZE ||
          test->settings->blksize > MAX_UDP_BLOCKSIZE))) {
      i_errno = IEUDPBLOCKSIZE;
      return -1;
    }
    ...
    /* Ensure that the client does not request to run longer than the server's
     * configured max */
    if ((test->max_server_duration > 0) &&
        (((test->duration + test->omit) > test->max_server_duration) ||
         (test->duration == 0))) {
    ...
    iperf_size_t total_requested_rate = test->num_streams *
                                        test->settings->rate *
    ...
    total_requested_rate = test->num_streams * test->settings->fqrate *
                           (test->mode == BIDIRECTIONAL ? 2 : 1);
    if (test->settings->bitrate_limit &&
        total_requested_rate > test->settings->bitrate_limit) {
      i_errno = IETOTALRATE;
...
static int get_results(struct iperf_test *test) {
  ...
  // Init as seems to not be au-initialized to 0 by default, maybe because later
  // init is only under if statement
...
static void iperf_print_intermediate(struct iperf_test *test) {
   ...
   *
   * The user can still opt in for all measurement data via the
   * --json-stream-full-output option.
   */
  discard_json = test->json_stream == 1 && !test->json_stream_full_output &&
        ...
        if (sp->sender == stream_must_be_sender) {
              ...
              if (test->json_output)
                cJSON_AddItemToObject(
                    ...
                    iperf_json_printf(
                        ...
                        "%d  bits_per_second: %f  retransmits: %d  reorder:  "
                        "%d  max_snd_cwnd:  %d  max_snd_wnd:  %d  max_rtt:  %d "
                        ...
                        (int64_t)sp->result->stream_reorder,
          ...
          if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
            ...
            if (!test->json_output) {
              ...
              } else if (receiver_packet_count - receiver_omitted_packet_count >
                             0 &&
              ...
              if (test->role == 's' && sp->sender) {
                ...
                if (test->omit == 0) {
                  iperf_printf(test, report_bw_udp_format, sp->socket, mbuf,
                               ...
                               sp->jitter * 1000.0, sp->cnt_error,
                               receiver_packet_count, lost_percent,
                               report_receiver);
    ...
    if (test->num_streams > 1 || test->json_output) {
      ...
      if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
        ...
        if (test->json_output) {
          ...
          double sent_bandwidth =
              sender_time > 0 ? ((double)total_sent * 8 / sender_time) : 0.0;
          double recv_bandwidth =
              receiver_time > 0 ? ((double)total_received * 8 / receiver_time)
                                : 0.0;
          /*
          ...
          cJSON_AddItemToObject(
              ...
              iperf_json_printf(
                  ...
                  (int64_t)total_sent, sent_bandwidth, (double)0.0, (int64_t)0,
                  (int64_t)sender_total_packets, (double)0.0, 1));
          cJSON_AddItemToObject(
              test->json_end, sum_received_name,
              iperf_json_printf(
                  "start: %f  end: %f  seconds: %f  bytes: %d  "
                  ...
                  (double)start_time, (double)receiver_time,
                  (double)receiver_time, (int64_t)total_received,
                  recv_bandwidth, (double)avg_jitter * 1000.0,
                  (int64_t)lost_packets, (int64_t)receiver_total_packets,
...
static void print_interval_results(struct iperf_test *test,
  ...
  if (test->mode == BIDIRECTIONAL) {
    snprintf(mbuf, sizeof(mbuf), "[%s-%s]", sp->sender ? "TX" : "RX",
             test->role == 'c' ? "C" : "S");
  } else {
    mbuf[0] = '\0';
    zbuf[0] = '\0';
      ...
      if (test->json_output)
        cJSON_AddItemToArray(
            ...
            iperf_json_printf(
                ...
                " %d  rtt:  %d  rttvar: %d  pmtu: %d  reorder: %d  omitted: %b "
                "sender: %b",
                ...
                (int64_t)irp->reorder, irp->omitted, sp->sender));
      else {
...
struct iperf_stream *iperf_new_stream(struct iperf_test *test, int s,
  ...
  if (test->protocol->id == Pudp && test->settings->gso &&
    ...
    size = test->settings->gso_bf_size;
  if (test->protocol->id == Pudp && test->settings->gro &&
  ...
  sp->buffer = (char *)mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED,
                            sp->buffer_fd, 0);
...
int iperf_set_control_keepalive(struct iperf_test *test) {
  ...
  if (test->settings->cntl_ka) {
    ...
    // Seems that at least in Windows WSL2, TCP keepalive retries full interval
